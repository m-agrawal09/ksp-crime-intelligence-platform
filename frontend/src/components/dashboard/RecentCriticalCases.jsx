...
const RecentCriticalCases = ({ cases }) => {
  ...
  const getRiskBadge = (risk) => {
    switch (risk) {
      case "CRITICAL":
        return "bg-red-500/10 text-red-400 border border-red-500/20 font-black tracking-wide";
      case "HIGH":
        return "bg-amber-500/10 text-amber-400 border border-amber-500/20 font-bold tracking-wide";
      case "MEDIUM":
      default:
        return "bg-blue-500/10 text-blue-400 border border-blue-500/20 font-semibold";
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default:
        return "text-slate-400 bg-slate-900 border border-slate-800/40";
    }
  };

  return (
    <div className="rounded-[4px] border border-slate-800/60 bg-slate-900/60 p-6 sm:p-8 flex flex-col h-[560px] shadow-lg">
      <div className="flex items-center justify-between border-b border-slate-800/60 pb-4 mb-6">
        ...
        <span className="rounded-full bg-slate-850 px-2.5 py-0.5 font-mono text-[9px] text-slate-500">
          Source: CaseMaster
        </span>
      </div>
      ...
      <div className="flex-1 overflow-auto scrollbar-thin">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-800/60 text-[9px] font-mono text-slate-500 uppercase tracking-widest">
              <th className="py-4 px-5">Crime Number / CaseNo</th>
              <th className="py-4 px-5">Jurisdiction (Unit)</th>
              <th className="py-4 px-5">Sections & Acts</th>
              <th className="py-4 px-5">Status</th>
              <th className="py-4 px-5 text-right">Risk Index</th>
              ...
              return (
                <React.Fragment key={c.CaseMasterID}>
                  ...
                  <tr className="hover:bg-slate-800/20 transition-colors group">
                    ...
                    <td className="py-5 px-5 text-slate-300">
                    ...
                    <td className="py-5 px-5">
                      ...
                      <div className="text-[9px] text-slate-500 mt-1 flex items-center gap-1.5 font-mono">
                        <FaCalendarAlt className="text-[8px] text-slate-600" /> Filed: {c.CrimeRegisteredDate}
                      </div>
                    </td>
                    <td className="py-5 px-5">
                      <span className={`rounded-[4px] px-2 py-0.5 text-[9px] tracking-wide font-bold ${getStatusColor(c.CaseStatusName)}`}>
                        {c.CaseStatusName}
                      </span>
                    </td>
                    <td className="py-5 px-5 text-right">
                      <span className={`rounded-[4px] px-2 py-0.5 text-[9px] tracking-wider ${getRiskBadge(c.risk_index)}`}>
                        {c.risk_index}
                      </span>
                    </td>
                    <td className="py-5 px-5 text-center">
                      <button
                        ...
                        className="p-1 rounded-[4px] bg-slate-800/40 hover:bg-slate-750 text-slate-400 hover:text-white transition-colors cursor-pointer"
                      >
